...
export function addLanguage() {
  ...
  div.innerHTML = `
    <div class="flex justify-between items-center mb-3">
      <span class="text-sm font-medium text-stone-500">Idioma #${index + 1}</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="space-y-2">
        <label class="text-sm font-medium text-stone-600">Idioma *</label>
        <input type="text" name="lang-name-${index}" placeholder="Ej: Español, Inglés..." class="w-full px-3 py-2 border border-stone-200 rounded-md focus:outline-none focus:ring-2 focus:ring-stone-300" data-action="lang-input" data-index="${index}" data-field="language">
      </div>
      <div class="space-y-2">
        <label class="text-sm font-medium text-stone-600">Nivel *</label>
        <input type="text" name="lang-level-${index}" placeholder="Ej: Nativo, B2, C1..." class="w-full px-3 py-2 border border-stone-200 rounded-md focus:outline-none focus:ring-2 focus:ring-stone-300" data-action="lang-input" data-index="${index}" data-field="level">
      </div>
    </div>
  `;
